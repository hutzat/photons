        ...
        function obj=lifetime_histo(obj,file_in_key,file_out_key,resolution)
            ...
            switch obj.header.MeasurementMode
                ...
                case 3
                    ...
                    obj.(file_out_key)=struct('time',time,'lifetime',histo,'resolution',resolution);
            end
        end
        function obj=FLID(obj,file_in_key,file_out_key,resolution,pulses_per_bin)
...
%             plot(time,g2_noise_sub)
        ...
        function obj = photon_corr(obj, file_in_key,file_out_key,channels,time_bounds,lag_precision,lag_offset,use_dll)
            ...
            if use_dll
                ...
                if obj.header.MeasurementMode == 3
                    sync_period = 1E12/obj.header.SyncRate;      % in picoseconds
        ...
        function obj=fit_lifetime_biexponential(obj,lifetime_id,resolution)
                %% prepares the lifetime (applying offset and cleaning) and then fits the lifetime with a biexponential.
                  time=obj.(lifetime_id).time;
                  histo=obj.(lifetime_id).lifetime;
                  
                  options = fitoptions('exp2');
                  options.Lower = [0 -5.0000E-04 0 -5.0000E-04];
                  options.Upper = [1 0 1 0];
                  
                  
                  [a,b]=max(histo);
                  
                  histo_corr=[histo(b:end),histo(1:b-10)];
                  histo_corr=histo_corr(histo_corr ~=0);
                  histo_corr=histo_corr-mean(histo_corr(end-10:end))
                  histo_corr=histo_corr/max(histo_corr);
                  n=numel(histo_corr);
                  time_corr=[0:n-1]*obj.(lifetime_id).resolution;
                  exp2=fit(time_corr',histo_corr','exp2',options);
                  
                  
          obj.(lifetime_id(:)).('exp2')=struct('fit',exp2,'time',time_corr,'lifetime',histo_corr);
